...
namespace CompanionClaude
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // ===== PICKUP SCENE: PIPER-EXACT TOPICS (NO CUSTOM TOPICS YET) =====
            ...
            pickupAction1.PlayerPositiveResponse.SetTo(piperPickup_PPos);
            pickupAction1.NpcPositiveResponse.SetTo(piperPickup_NPos);
            pickupAction1.PlayerNegativeResponse.SetTo(piperPickup_PNeg);
            ...
            // PICKUP GREETING 1: First time (HasBeen=0) - Piper text
            ...
            // PICKUP GREETING 2: Returning (HasBeen=1) - Piper text (same line, different conditions)
            ...
            try
            {
                bool enableAstraPickupTts = false; // set true when ready to swap from Piper to Astra
                ...
                if (System.IO.File.Exists(lipGen) && System.IO.File.Exists(xwmEncode))
                {
                    ...
                    var pickupLines = new (FormKey formKey, string text)[] {
                        (pickupGreeting.FormKey, "Ready to move out?"),
                        (formerPickupGreeting.FormKey, "I can run point again. Ready?")
                    };

                    string dstNpcPickup = System.IO.Path.Combine(dstBase, "NPCFPiper");
