...
Scriptname Fragments:Quests:QF_COMAstraMQ302ALT_000009A1 Extends Quest Hidden
...
Function MQ302AltMarkDisplayedObjectivesFailed()
  int idx = 5
  while idx <= 100
    if IsObjectiveDisplayed(idx)
      SetObjectiveFailed(idx, true)
    endif
    idx += 5
  endwhile
EndFunction

Bool Function MQ302AltIsInstituteDestroyedHardFail()
  if PlayerInstitute_Destroyed && PlayerInstitute_Destroyed.GetValue() >= 1.0
    return true
  endif
  if MQComplete && MQComplete.GetValue() >= 1.0
    return true
  endif
  if MQ302 && MQ302.GetStage() >= 850
    return true
  endif
  return false
EndFunction

Bool Function MQ302AltShouldRouteRecovery()
  if !EnableRecoveryBranch || HardFailInstituteDestroyed
    return false
  endif

  if MQ302 && (MQ302.IsRunning() || MQ302.GetStage() >= 10)
    return true
  endif
  if MQ302Min && (MQ302Min.IsRunning() || MQ302Min.GetStage() >= 10)
    return true
  endif
  if MQ302BoS && (MQ302BoS.IsRunning() || MQ302BoS.GetStage() >= 355)
    return true
  endif
  if MQ302RR && (MQ302RR.IsRunning() || MQ302RR.GetStage() >= 325)
    return true
  endif
  if MQ302Post && MQ302Post.IsRunning()
    return true
  endif
...
Function MQ302AltRouteToRecoveryBranch()
  int targetStage = RecoveryEntryStage
  if targetStage <= 10
    targetStage = 95
  endif

  RecoveryBranchActive = true
  RecoveryBranchRouted = true
  if DebugTraceMq302AltShellStages
    Debug.Trace(self + " MQ302ALT recovery route active -> stage " + targetStage)
  endif

  if !GetStageDone(targetStage)
    SetStage(targetStage)
  endif
EndFunction

Function MQ302AltApplyInstituteDestroyedHardFail()
  HardFailInstituteDestroyed = true
  RecoveryBranchActive = false
  RecoveryBranchRouted = false
  MQ302AltMarkDisplayedObjectivesFailed()
  SetObjectiveDisplayed(5, true)
  SetObjectiveFailed(5, true)

  if DebugTraceMq302AltShellStages
    Debug.Trace(self + " MQ302ALT hard fail: Institute is already destroyed in this save")
  endif
...
RecoveryBranchActive = false
RecoveryBranchRouted = false
HardFailInstituteDestroyed = false

if MQ302AltIsInstituteDestroyedHardFail()
  MQ302AltApplyInstituteDestroyedHardFail()
  return
endif

if MQ302AltShouldRouteRecovery()
  MQ302AltRouteToRecoveryBranch()
  return
endif
...
Int Property RecoveryEntryStage Auto
...
Bool Property EnableRecoveryBranch Auto
Bool Property RecoveryBranchActive Auto
Bool Property RecoveryBranchRouted Auto
Bool Property HardFailInstituteDestroyed Auto
