...
static class DumpMQ102Dialogue
{
    public static void Run()
    {
        ...
        var targetEditorIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            { "MQ102", "Min00", "Min01", "DialogueCodsworthPostWar" };
        ...
        foreach (var quest in quests)
        {
            ...
            // Dump DialogTopics
            Console.WriteLine($"\n  --- DIALOG TOPICS ({(quest.DialogTopics?.Count ?? 0)}) ---");
        ...
        var questEdidCache = new Dictionary<FormKey, string>();
        foreach (var q in env.LoadOrder.PriorityOrder.WinningOverrides<IQuestGetter>())
            questEdidCache[q.FormKey] = q.EditorID ?? "(no edid)";

        // Text hunt: find which quest owns the post-vault Codsworth conversation
        Console.WriteLine($"\n\n{"=",80}");
        Console.WriteLine("TEXT HUNT: prompts/responses about husband/killers/what happened...");
        Console.WriteLine($"{"=",80}");
        foreach (var topic in topicCache.Values)
        {
            foreach (var rb in topic.Responses)
            {
                var prompt = rb.Prompt?.String ?? "";
                bool hit = prompt.Contains("husband", StringComparison.OrdinalIgnoreCase)
                    || prompt.Contains("What happened", StringComparison.OrdinalIgnoreCase)
                    || prompt.Contains("Everything's dead", StringComparison.OrdinalIgnoreCase);
                if (!hit)
                {
                    foreach (var r in rb.Responses)
                    {
                        var txt = r.Text?.String ?? "";
                        if (txt.Contains("mum", StringComparison.OrdinalIgnoreCase)
                            || txt.Contains("it's really you", StringComparison.OrdinalIgnoreCase))
                        { hit = true; break; }
                    }
                }
                if (hit)
                {
                    Console.WriteLine($"  HIT Topic: {topic.EditorID} ({topic.FormKey}) Quest={(topic.Quest != null && !topic.Quest.IsNull && questEdidCache.TryGetValue(topic.Quest.FormKey, out var qe) ? qe + " " + topic.Quest.FormKey : "none")} Prompt=\"{prompt}\"");
                    break;
                }
            }
        }
        // Search ALL topics that point at these quests (FO4 links topic->quest)
        Console.WriteLine($"\n\n{"=",80}");
        Console.WriteLine("SEARCHING ALL TOPICS REFERENCING MQ102/Min00/Min01...");
        Console.WriteLine($"{"=",80}");
        var questFKs0 = quests.Select(q => q.FormKey).ToHashSet();
        foreach (var topic in topicCache.Values)
        {
            if (topic.Quest != null && !topic.Quest.IsNull && questFKs0.Contains(topic.Quest.FormKey))
            {
                DumpTopic(topic, "  ");
            }
        }
