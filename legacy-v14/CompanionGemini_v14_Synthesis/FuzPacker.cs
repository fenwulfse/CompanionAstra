    ...
    public static class FuzPacker
    {
        ...
        public static void PackFuzFile(string xwmPath, string lipPath, string fuzPath)
        {
            ...
            using (var bw = new BinaryWriter(fs))
            {
                ...
                // Write Fallout 4 .fuz layout: FUZE + version + lip size + lip + audio
                bw.Write(FUZE_MAGIC);
                bw.Write(FALLOUT4_FUZE_VERSION);
        ...
        /// <param name="lipGenPath">Path to LipGenerator.exe</param>
        /// <param name="xwmEncodePath">Path to xwmaencode.exe</param>
            ...
            string? xwmEncodePath = null)
        {
            ...
            // Set default tool paths relative to the project structure
            string projectRoot = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", ".."));
            lipGenPath ??= Path.Combine(projectRoot, "Tools", "LipGenerator.exe");
            xwmEncodePath ??= Path.Combine(projectRoot, "Tools", "xwmaencode.exe");
            ...
            finally
            {
                ...
                if (File.Exists(lipPath) && Path.GetFullPath(Path.GetDirectoryName(lipPath)!) != Path.GetFullPath(Path.GetDirectoryName(fuzPath)!))
                {
                    try { File.Delete(lipPath); } catch { }
                }
                if (File.Exists(xwmPath) && Path.GetFullPath(Path.GetDirectoryName(xwmPath)!) != Path.GetFullPath(Path.GetDirectoryName(fuzPath)!))
                {
        ...
        {
            UnpackFuzFileFallout4(fuzPath, outputXwmPath, outputLipPath);
            return;
        ...
        private static void UnpackFuzFileFallout4(string fuzPath, string outputXwmPath, string outputLipPath)
        {
            if (!File.Exists(fuzPath))
                throw new FileNotFoundException($"FUZ file not found: {fuzPath}");
            ...
            using (var br = new BinaryReader(fs))
            {
                ...
                // Read version (unused, but required for correct offset)
                var version = br.ReadUInt32();
                if (version != FALLOUT4_FUZE_VERSION)
                {
                    Console.WriteLine($"Warning: Unexpected FUZE version {version}. Expected {FALLOUT4_FUZE_VERSION}.");
                }
                ...
                // Remaining bytes are audio data
                var remaining = fs.Length - fs.Position;
                var audioData = br.ReadBytes((int)remaining);
                File.WriteAllBytes(outputXwmPath, audioData);
                Console.WriteLine($"âœ“ Extracted audio: {outputXwmPath} ({remaining} bytes)");
            }
        }
    }
}
